...
updateStatusesRegularly groupId (DataUpdateIntervalSeconds updateInterval) excludeList =
  addNamespace "update-statuses" $ pass <$> infinitely (updateWithDurationObservation groupId excludeList >> Time.sleep (Seconds (fromIntegral updateInterval)))
...
updateWithDurationObservation groupId excludeList =
  observeDuration $ do
    ...
    addContext "numResults" (length results) $ logInfo "Done updating build statuses"
