...
runConfig :: Config -> Sem (Reader SharedProjects : Reader UserAgent : Reader ApiToken : Reader (Url GitlabHost) : Reader MaxConcurrency : Reader DataUpdateIntervalSeconds : Reader ExtraProjectsList : Reader ProjectExcludeList : Reader [Id Group] ': r) a -> Sem r a
runConfig Config {..} =
  runReader groupId
